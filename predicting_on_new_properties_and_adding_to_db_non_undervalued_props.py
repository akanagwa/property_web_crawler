...
conn = sqlite3.connect(r'C:\Users\arnie\Desktop\business_analytics\PYTHON_FOR_ANALYTICS\Web_Scraping\Latest_version\oklahoma_homes_sale.db')
...
median_price_byzipcode=current.groupby('zip_code')[['price']].median().reset_index().rename({'price':'median_homeprice_byzip'},axis=1)
...
###reading in nulls 
nulls=pd.read_csv(r'C:\Users\arnie\Desktop\business_analytics\PYTHON_FOR_ANALYTICS\Web_Scraping\Latest_version\null.csv')
...
new4['median_homeprice_byzip']=new4['median_homeprice_byzip'].fillna(nulls['median_homeprice_byzip'].median())
...
new4['percapitmedianzipprice']=new4['percapitmedianzipprice'].fillna(nulls['percapitmedianzipprice'].median())
...
##call saved model 
import joblib 
rf_model=joblib.load(r'C:\Users\arnie\Desktop\business_analytics\PYTHON_FOR_ANALYTICS\Web_Scraping\Latest_version\rf_final.pkl')
...
a=new5['bed_rooms']>=5 
b=new5['price_per_sq_ft']<130 # less than 130 is the ideal price per square foot for a good deal on a duplex 
...
try: 
    ...
    new_entry2=new_entry[~new_entry['unique_id'].isin(undervalued_sort['unique_id'])] ##only return properties that are undervalued, add the rest to the database
